...
async function main() {
  ...
  const task4 = await task_service.createTask(john.id, {title: "Study project for full stack", description: "Finish project", dueDate: new Date(2023, 6, 9, 12, 0).toISOString()})
  const task5 = await task_service.createTask(john.id, {title: "Presentation for full stack", description: null, dueDate: new Date(2023, 6, 10).toISOString()})
